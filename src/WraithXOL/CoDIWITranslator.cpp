...
std::unique_ptr<XImageDDS> CoDIWITranslator::TranslateIWI(const std::unique_ptr<uint8_t[]>& IWIBuffer, uint32_t IWIBufferSize)
{
    ...
    if (Header.Version == 0x8 || Header.Version == 0x9) { Reader.SetPosition(8); }
    ...
    if (Header.Magic[0] == 0x49 && Header.Magic[1] == 0x57 && Header.Magic[2] == 0x69)
    {
        ...
        else if (Header.Version == 0x6 || Header.Version == 0x8 || Header.Version == 0x9)
        {
            // Small mips
            if (Header.Version == 0x6) { Reader.SetPosition(0xC); }
            if (Header.Version == 0x8 || Header.Version == 0x9) { Reader.SetPosition(0x10); }

            auto SmallMips = Reader.Read<IWISmallMips>();

            if (SmallMips.MipOffset1 == SmallMips.MipOffset2 && SmallMips.MipOffset1 == SmallMips.MipOffset3 && SmallMips.MipOffset1 == SmallMips.MipOffset4)
            {
                // Calculate the offset
                switch (Info.ImageFormat)
                {
                case 0xB: OffsetToDump = (int32_t)(Reader.GetLength() - ((Info.ImageWidth * Info.ImageHeight * 4) / 8)); break;
                case 0xC: OffsetToDump = (int32_t)(Reader.GetLength() - (Info.ImageWidth * Info.ImageHeight)); break;
                case 0xD: OffsetToDump = (int32_t)(Reader.GetLength() - (Info.ImageWidth * Info.ImageHeight)); break;
                default: OffsetToDump = (int32_t)Reader.GetPosition(); break;
                }
            }
            else if (SmallMips.MipOffset1 == SmallMips.MipOffset2 || SmallMips.MipOffset1 == SmallMips.MipOffset4)
            {
                // Grab ofset
                OffsetToDump = (int32_t)Reader.GetPosition();
            }
        ...
        if (OffsetToDump > -1 && SizeToDump > -1)
        {
            ...
            // Write the header
            Image::WriteDDSHeaderToStream(ImageBuffer, Info.ImageWidth, Info.ImageHeight, 1, ImageDataFormat, ResultSize);
