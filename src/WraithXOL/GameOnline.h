...
struct GameExportConfig
{
    ...
    void ValidateConfig()
    {
        // If all are disabled, set defaults
        if (!OBJ && !XNA && !SMD && !XME && !SEModel && !Maya)
            Maya = true;
        if (!PNG && !DDS)
            PNG = true;
        if (!XAnimsWAW && !XAnimsBO && !SEAnims)
            SEAnims = true;
    }
};
...
public:
    ...
    static void ExtractAssets(bool Anims, bool Models, bool Images, bool Sounds, bool ListOnly, std::vector<std::string> SearchFilter);
