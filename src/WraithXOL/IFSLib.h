...
public:
    ...
    void AddPackage(const std::string& PackagePath, bool FileNamesOnly);
    // Parse and load an IFS file with the list file
    std::vector<std::string> ParsePackage(const std::string& PackagePath, bool FileNamesOnly);
    // Parse and load all available IFS packages in the path
    void MountIFSPath(const std::string& IFSPath, bool FileNamesOnly);
...
private:
    ...
    void LoadPackageInternal(const std::string& PackagePath, std::vector<std::string>& LoadedListFile, bool Audio = false, bool FileNamesOnly = false);
